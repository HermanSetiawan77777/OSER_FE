...
export class ProjectServices {
  ...
  getProjects() {
    return this.http.get<Projects>(
      `${environment.apiUrl}/project/ViewProjectAll`
    );
  }

  getMyProjects(ownerId: string) {
    return this.http.get<Projects>(
      `${environment.apiUrl}/project/ViewProjectByOwner/${ownerId}`
    );
  }
  ...
  getById(id: string) {
    ...
    return this.http.get<Projects>(`${environment.apiUrl}/project/${id}`, {
      headers: header,
    });
  }
  ...
  registerProject(project: Projects, file: File): Observable<any> {
    ...
    return this.http.post(
      `${environment.apiUrl}/project/InsertProject`,
      formData,
      { headers: header }
    );
  }
  ...
  detailProject(id: string) {
    return this.http.get<Projects>(
      `${environment.apiUrl}/project/ViewProjectDetail/${id}`
    );
  }
  ...
  takeProjects(assignmentid: string, category: string) {
    ...
    header = header.set('TOKEN', localStorage.getItem('token').slice(1, -1));
    return this.http.post(
      `${environment.apiUrl}/schedule/InsertSchedule`,
      { assignmentid, category },
      { headers: header }
    );
  }
  ...
  requestPrice(assignmentid: string, category: string) {
    ...
    header = header.set('TOKEN', localStorage.getItem('token').slice(1, -1));
    return this.http.post(
      `${environment.apiUrl}/requestprice/InsertRequestPrice`,
      { assignmentid, category },
      { headers: header }
    );
  }
  ...
  updateProject(project: Projects, id, file: File) {
    ...
    header = header.set('TOKEN', localStorage.getItem('token').slice(1, -1));
    return this.http.post(
      `${environment.apiUrl}/project/UpdateProject/${id}`,
      formData,
      { headers: header }
    );
  }
  ...
  deleteProjects(id) {
    ...
    return this.http.delete(
      `${environment.apiUrl}/project/DeleteProject/${id}`,
      { headers: header }
    );
  }
}
