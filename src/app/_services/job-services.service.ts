...
export class JobServicesService {
  ...
  getServices() {
    return this.http.get<Services>(
      `${environment.apiUrl}/services/ViewServicesListAll`
    );
  }

  getMyServices(ownerId: string) {
    return this.http.get<Services>(
      `${environment.apiUrl}/Services/ViewServicesListbyOwner/${ownerId}`
    );
  }
  ...
  getById(id: string) {
    ...
    return this.http.get<Services>(`${environment.apiUrl}/services/${id}`, {
      headers: header,
    });
  }
  ...
  registerServices(services: Services, file: File): Observable<any> {
    ...
    header = header.set('TOKEN', localStorage.getItem('token').slice(1, -1));
    return this.http.post(
      `${environment.apiUrl}/services/InsertServices`,
      formData,
      { headers: header }
    );
  }
  ...
  detailServices(id: string) {
    return this.http.get<Services>(
      `${environment.apiUrl}/Services/ViewServicesDetail/${id}`
    );
  }
  ...
  updateServices(id, services: Services, file: File) {
    ...
    header = header.set('TOKEN', localStorage.getItem('token').slice(1, -1));
    return this.http.post(
      `${environment.apiUrl}/services/UpdateServices/${id}`,
      formData,
      { headers: header }
    );
  }
  ...
  deleteServices(id) {
    ...
    return this.http.delete(
      `${environment.apiUrl}/services/DeleteServices/${id}`,
      { headers: header }
    );
    // .pipe(map(x => {
