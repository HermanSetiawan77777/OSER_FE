...
export class ScheduleServices {
  ...
  getScheduleByUserId(userId: string) {
    return this.http.get<Schedule>(`${environment.apiUrl}/schedule/viewSchedulebyUserID/${userId}`);
  }

  getScheduleByOwnerId(ownerId: string) {
    return this.http.get<Services>(`${environment.apiUrl}/schedule/ViewScheduleyByOwner/${ownerId}`);
  }

  getScheduleDetail(id: string){
    return this.http.get<Services>(`${environment.apiUrl}/schedule/ViewScheduleDetail/${id}`);
  }

  getById(id: string) {
    let header = new HttpHeaders();
    header = header.set('TOKEN', localStorage.getItem('token').slice(1,-1));
    return this.http.get<Schedule>(`${environment.apiUrl}/schedule/${id}`,{ headers: header });
  }
